...
def csv_append(user_query='-', item_count='-', name='-', item_page='-', rating='-', review='-', is1='-', is15='-', is2='-', is25='-', is3='-', price='-'):
    global is_first
    global csv_index
...
def slugify(img_name):
    ...
    sluged_name = sluged_name.replace('|', ' ').replace('&', ' ').replace(':', ' ') # ASCII Range can be checked instead, More info needed about how will the images \
    # be used.
...
def scrape_query(user_query, page_count):
    ...
    if len(container) > 0:
        ...
        for item in container:
            ...
            try:
                image_srcset =  [(img_link.split(' ')[0], img_link.split(' ')[1]) for img_link in item.a.img['srcset'].split(', ')]
            except:
                image_srcset = None
            try: 
            ...
            csv_append(user_query=user_query, item_count= item_count, name=img_alt, item_page=item_page, rating=rating, review=review_count, is1=is1, is15=is15, is2=is2, is25=is25, is3=is3, price=price)
            is_first = False
    else:
        item = None
        return item
...
if __name__ == "__main__":
    ...
    user_query = input("Enter your Product: ").replace(' ', '+') # More filtering is needed | search 'mobile' to get all mobile data scraped
